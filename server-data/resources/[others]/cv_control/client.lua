...
function MainMenu()
    ...
    Menu.addButton("Limiteur de vitesse", "MenuSpeedo", 50)
...
function MenuSpeedo()
    if regulateur == 50 then
        options.menu_subtitle = "LIMITEUR ACTUEL :  AUCUN"
    else
        options.menu_subtitle = "LIMITEUR ACTUEL : ~g~" ..regulateur..  "~s~ Km/h"
    end
    ClearMenu()
    Menu.addButton("Augmenter le limiteur", "uprregu", nil)
    Menu.addButton("Baisser le limiteur", "downrregu", nil)
    if regulateur > 50 then
        Menu.addButton("Enlever le limiteur", "suprregu", nil)
    end
    Menu.addButton("Retour", "MainMenu", nil)
end
    ...
    if (GetOnscreenKeyboardResult()) then
        ...
        if not vitesse ~= nil then
            ...
            if currSpeed > res then
                Notify("~r~Tu ne peux pas car ta vitesse est supérieure à ~g~60 ~r~Km/h.")
            else
                speedo(res)
                regulateur = res
                MainMenu()
...
function uprregu()
    ...
    if currSpeed > regulateur then
        Notify("~r~Tu ne peux pas car ta vitesse est supérieure à celle du ~g~limiteur")
    else
