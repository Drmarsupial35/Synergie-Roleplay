...
function TeleportPlayer(pos)
	...
	if #Values["goal"] > 1 then
		...
		function(data, menu)
			...
			if action == "Sortie" or action == "Sécurité" then

				if PlayerData.job.name ~= "police" then
					ESX.ShowNotification("Vous n'avez pas les clés pour vous rendre ici !")
					return
				end
			end
...
Citizen.CreateThread(function()
	local blip = AddBlipForCoord(Config.Teleports["Sortie"]["x"], Config.Teleports["Sortie"]["y"], Config.Teleports["Sortie"]["z"])
    ...
    AddTextComponentString('Pénitencier de Bolingbroke')
