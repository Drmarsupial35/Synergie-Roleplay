...
ESX.RegisterServerCallback('esx_policejob:getVehicleInfos', function(source, cb, plate)
	...
	MySQL.Async.fetchAll('SELECT job FROM owned_vehicles WHERE plate = @plate', {
		['@plate'] = plate
	}, function(result)
		...
		if result[1] ~= nil and result[1].job ~= '' then
			MySQL.Async.fetchAll('SELECT label FROM jobs WHERE name = @name', {
				['@name'] = result[1].job
			}, function(result2)
			...
			MySQL.Async.fetchAll('SELECT owner FROM owned_vehicles WHERE plate = @plate', {
				...
				if result2[1] then
					MySQL.Async.fetchAll('SELECT name, firstname, lastname FROM users WHERE identifier = @identifier',  {
						['@identifier'] = result2[1].owner
					}, function(result3)

						if Config.EnableESXIdentity then
							retrivedInfo.owner = result3[1].firstname .. ' ' .. result3[1].lastname
						else
							retrivedInfo.owner = result3[1].name
						end
