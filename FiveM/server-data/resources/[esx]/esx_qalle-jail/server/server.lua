	...
	if xPlayer["job"]["name"] == "police" then
		...
		if GetPlayerName(jailPlayer) ~= nil then
			...
			if jailTime ~= nil then
				...
				TriggerClientEvent("esx:showNotification", src, GetPlayerName(jailPlayer) .. " emprisonné pendant " .. jailTime .. " minutes!")

				if args[3] ~= nil then
					GetRPName(jailPlayer, function(Firstname, Lastname)
						TriggerClientEvent('chat:addMessage', -1, { args = { "JUGE",  Firstname .. " " .. Lastname .. " est en prison pour : " .. args[3] }, color = { 249, 166, 0 } })
					end)
				end
			else
			...
			TriggerClientEvent("esx:showNotification", src, "Cette personne n'est pas connectée !")
		end
	else
		TriggerClientEvent("esx:showNotification", src, "Vous n'êtes pas policier !")
	end
end)
...
AddEventHandler("esx-qalle-jail:jailPlayer", function(targetSrc, jailTime, jailReason)
	...
	GetRPName(targetSrc, function(Firstname, Lastname)
		TriggerClientEvent('chat:addMessage', -1, { args = { "JUGE",  Firstname .. " " .. Lastname .. " est en prison pour : " .. jailReason }, color = { 249, 166, 0 } })
	end)
...
AddEventHandler("esx-qalle-jail:unJailPlayer", function(targetIdentifier)
	...
	TriggerClientEvent("esx:showNotification", src, xPlayer.name .. " libéré(e) !")
end)
...
AddEventHandler("esx-qalle-jail:prisonWorkReward", function()
	...
	TriggerClientEvent("esx:showNotification", src, "Merci ! Voici de l'argent pour vous acheter à manger.")
end)
