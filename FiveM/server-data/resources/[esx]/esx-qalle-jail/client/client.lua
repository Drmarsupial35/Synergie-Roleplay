...
function JailLogin()
	...
	ESX.ShowNotification("Votre peine de prison n'est toujours finie. Les gardes vous ont renvoyés dans votre cellule!")
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			ESX.ShowNotification("Il vous reste " .. jailTime .. " minutes à passer en prison !")
		...
		while jailTime > 0 do
			...
			for posId, v in pairs(Packages) do
				...
				if DistanceCheck <= 10.0 then
					...
					local PackageText = "Paquet"

					if not v["state"] then
						PackageText = "déjà ramaser"
					end
...
function PackPackage(packageId)
	...
	while Packaging do
		...
		if not IsPedUsingScenario(PlayerPedId(), "PROP_HUMAN_BUM_BIN") then
			...
			ESX.ShowNotification("Annulation !")
		...
		if PackPercent >= 100 then
			...
			ESX.Game.Utils.DrawText3D(Package, "Paquet... " .. math.ceil(tonumber(PackPercent)) .. "%", 0.4)
		end
...
function DeliverPackage(packageId)
	...
	while Packaging do
		...
		if not IsEntityAttachedToEntity(packageId, PlayerPedId()) then
			...
			ESX.Game.Utils.DrawText3D(DeliverPosition, "[E] Lâcher le colis", 0.4)
...
function OpenJailMenu()
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'jail_prison_menu',
		{
			...
			align    = 'center',
			elements = {
				{ label = "Emprisonner", value = "jail_closest_player" },
				{ label = "Libérer", value = "unjail_player" }
			}
		},
	function(data, menu)
		...
		if action == "jail_closest_player" then
			...
			ESX.UI.Menu.Open(
          		...
          		{
            		title = "Temps d'emprisonnement (en minutes)"
          		},
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("esx-qalle-jail:retrieveJailedPlayers", function(playerArray)
				...
				if #playerArray == 0 then
					ESX.ShowNotification("Votre prison est vide !")
					return
				end

				for i = 1, #playerArray, 1 do
					table.insert(elements, {label = "Prisonnier: " .. playerArray[i].name .. " | Temps d'incarcération: " .. playerArray[i].jailTime .. " minutes", value = playerArray[i].identifier })
				end
