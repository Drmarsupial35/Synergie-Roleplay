...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
		if IsControlJustPressed(1, 288) and allowProximityChange then
			local vprox
			if prox <= 2.0 then
				prox = 10.0
				vprox = "normal"
...
RegisterNetEvent('esx_customstatus:updateStatus')
AddEventHandler('esx_customstatus:updateStatus', function(status)
	SendNUIMessage({action = "updateStatus", status = status})
end)

RegisterNetEvent('esx_customstatus:updateWeight')
AddEventHandler('esx_customstatus:updateWeight', function(weight)
