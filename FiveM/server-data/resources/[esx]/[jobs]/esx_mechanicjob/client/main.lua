...
function CreateBlip()
	if ESX.PlayerData.job ~= nill and ESX.PlayerData.job.name == 'mechanic' and ESX.PlayerData.job.grade_name ~= 'recrue' then
		if BlipMechanic == nil then
			BlipMechanic = AddBlipForCoord(Config.Zones.Garage.Pos.x, Config.Zones.Garage.Pos.y, Config.Zones.Garage.Pos.z)

			SetBlipSprite (BlipMechanic, 402)
			SetBlipDisplay(BlipMechanic, 4)
			SetBlipScale  (BlipMechanic, 1.0)
			SetBlipColour (BlipMechanic, 5)
			SetBlipAsShortRange(BlipMechanic, true)

			BeginTextCommandSetBlipName("STRING")
			AddTextComponentString("Stocks d'outils")
...
-- Create Blips
Citizen.CreateThread(function()
	local blip = AddBlipForCoord(-205.520, -1306.669, 30.306)
