...
function MenuVehicleSpawner()
	...
	for i=1, #Config.Trucks, 1 do
		table.insert(elements, {label = "Camion Poubelle " .. i, value = Config.Trucks[i]})
	end
					...
					if dist <= 3.5 then
						if currentbag > 0 then
							...
							if iscurrentboss then
								...
								trashcollection = false
								truckdeposit = false
								ESX.ShowNotification("Travail fini, retournez au camion !")
...
Citizen.CreateThread(function()
	while true do
		...
		if truckdeposit then
			...
			dist = Vdist(plyCoords.x, plyCoords.y, plyCoords.z, trunk.x , trunk.y, trunk.z)
			if dist <= 2.0 then
			ESX.Game.Utils.DrawText3D(vector3(trunk.x , trunk.y ,trunk.z + 0.50), "[~g~E~s~] Jeter le sac dans le camion.", 1.0)
			end
		end
		...
		if trashcollection then
			...
			if dist <= 5.0 then
				if currentbag <= 0 then
					if iscurrentboss then
					ESX.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "[~g~E~s~] Déblayer", 1.0)
					else
					ESX.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "Ramassage effectué .. Retournez dans le camion", 1.0)
					end
				else
					ESX.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "[~g~E~s~] Ramasser le sac poubelle ["..currentbag.."/"..bagsoftrash.."]", 1.0)
				end
			end
		end
...
function CreateBlip()
	if PlayerData.job ~= nill and PlayerData.job.name == 'garbage' then
		if BlipGarbage == nil then
			...
			EndTextCommandSetBlipName(BlipGarbage)
		end
	else
		if BlipGarbage ~= nil then
			RemoveBlip(BlipGarbage)
			BlipGarbage = nil
		end
	end
end
